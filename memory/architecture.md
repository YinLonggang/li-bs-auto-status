...
## 2026-05-13 Dashboard 渐进展开与动态阶段数

- Dashboard 首页新增项目状态总览，先展示全部项目数量、整体完成率、逾期、重点问题和碰撞签核，再进入项目统计列表。
- 项目统计列表采用渐进式展开：点击项目行操作后在该项目下方展开单项目阶段状态，不再把单项目状态固定放在右侧面板。
- Dashboard 首页项目级详情进一步收口到项目统计列表展开行：Project Context、阶段轨、范围统计、详情筛选、模块泳道、检查项详情、重点问题、碰撞一页纸和导出入口都跟随所选项目行渐进展开，首页顶层只保留范围工具、项目总览、项目筛选和统计列表。
