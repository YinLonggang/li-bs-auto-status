...
- 阶段页不再并列展示阶段模板；设置 fallback 只保留检查模块状态，不展示模板源数据或负责人候选列表，避免旧信息架构回流。
...
- 导航入口收敛为 `配置中心`，不再在侧边栏拆分“项目列表 / 基础配置 / 阶段配置 / 配置管理”；配置中心同页承载范围选择、项目列表、项目基础信息、六阶段、阶段检查项、检查模块、检查项模板和模块负责人配置。
...
- 配置中心底部区域命名收口为“模块负责人配置”，采用横向表格展示模块、检查项数量、负责人摘要、状态和操作；移除页面右侧常驻负责人候选列表，候选人只在抽屉搜索中出现。
